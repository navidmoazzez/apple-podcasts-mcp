 ...
 *
 * The command IS the tool name. `get_reviews` runs as `get-reviews`, and the
...
function kindOf(schema: ZodTypeAny): { kind: FlagKind; choices?: string[]; repeatable: boolean } {
  ...
  switch (typeName) {
    ...
    case "ZodArray": {
      // An array of scalars is repeatable (`--shows a --shows b`). An array of
...
export function exitCodeFor(error: unknown): number {
  ...
  if (status === 401 || status === 403 || /auth|credential|token|vendor number/.test(text)) return EXIT.auth;
...
/**
 * `--select id,podcast.name` keeps only the named fields. Dotted paths descend,
 * arrays are traversed element-wise. This is what makes a long chart affordable.
 */
 ...
 * shaped-for-a-model output and is just as readable in a terminal. Summarising
 ...
 *
 * The package ships two binaries onto the same file. Printing
 * `apple-podcasts-mcp` at someone who typed `apple-podcasts-cli` hands them a
 * command that works but is not the one they have in their fingers.
 */
function binName(): string {
  const name = (process.argv[1] ?? "").split("/").pop() ?? "";
  return name.startsWith("apple-podcasts-cli") ? "apple-podcasts-cli" : "apple-podcasts-mcp";
}
...
function renderToolHelp(spec: AnyToolSpec): string {
  ...
  lines.push(``);
  // The source is on the line because it is the thing that decides whether a
  // command can work at all here: `public` needs nothing, `library` needs this
  // Mac, `reporter` needs credentials only a show owner has.
  lines.push(`Risk: ${spec.risk}, reads: ${spec.surface}`);
  lines.push(``);
  return lines.join("\n");
}
...
function renderToolList(tools: AnyToolSpec[]): string {
  ...
  lines.push(``);
  lines.push(`  * writes    ! overwrites a file, needs --confirm`);
  lines.push(``);
...
/**
 * The tools this process will expose, filtered exactly as `buildServer` filters
 * them: READ_ONLY hides the one write, and LIBRARY=0 hides the group that reads
 * personal data off this Mac.
 */
function visibleTools(guard: WriteGuard, config: Config): AnyToolSpec[] {
...
export async function runCli(argv: string[]): Promise<number> {
  ...
  const tools = visibleTools(guard, config);
  ...
  if (!spec) {
    // A tool that exists but is filtered out gets the reason, because "unknown
    // command" would send someone looking for a typo they did not make. The
    // order matters: export_subscriptions is both a write and a library tool,
    // and naming LIBRARY=0 at someone who set READ_ONLY sends them to the wrong
    // variable.
    const hidden = ALL_TOOLS.find((t) => t.name === name);
    const reason =
      hidden && guard.readOnly && hidden.risk !== "read"
        ? `${name} is unavailable: this server is running with APPLE_PODCASTS_READ_ONLY=1.`
        : `${name} is unavailable: this server is running with APPLE_PODCASTS_LIBRARY=0.`;
    emitError(
      new Error(hidden ? reason : `Unknown command '${command}'. Run \`${binName()} tools\`.`),
    );
    return 1;
  }
